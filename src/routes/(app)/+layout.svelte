<script lang="ts">
	...
	onMount(async () => {
		...
		const setupKeyboardShortcuts = () => {
			document.addEventListener('keydown', async (event) => {
				if (isShortcutMatch(event, shortcuts[Shortcut.SEARCH])) {
					...
					event.preventDefault();
                    (([...document.getElementsByClassName('copy-code-button')]?.at(-1) as HTMLElement | null))?.click();
				} else if (isShortcutMatch(event, shortcuts[Shortcut.COPY_LAST_RESPONSE])) {
					console.log('Shortcut triggered: COPY_LAST_RESPONSE');
					event.preventDefault();
                    (([...document.getElementsByClassName('copy-response-button')]?.at(-1) as HTMLElement | null))?.click();
					...
					event.preventDefault();
                    (([...document.getElementsByClassName('regenerate-response-button')]?.at(-1) as HTMLElement | null))?.click();
				}
			});
		};
		...
		if ($user?.role === 'admin' && ($settings?.showChangelog ?? true)) {
            showChangelog.set(($settings?.version ?? null) !== $config.version);
		}

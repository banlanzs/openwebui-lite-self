...
> **[!NOTE]**
> **This is the Lite version of Open WebUI** - A streamlined, frontend-backend separated variant optimized for local development. For the full production version, visit [open-webui/open-webui](https://github.com/open-webui/open-webui).
...
- **CORS errors**: Ensure `CORS_ALLOW_ORIGIN` includes `http://localhost:5173`
- **Port conflicts**: Use `netstat -ano | findstr :8080` (Windows) or `lsof -i :8080` (Linux/macOS) to find conflicting processes
- **Frontend not connecting**: Verify backend is running and accessible at `/health` endpoint

For detailed setup instructions, see [lite-start.md](./lite-start.md) or [start.md](./start.md).
...
Encountering connection issues? Visit the documentation for troubleshooting.
